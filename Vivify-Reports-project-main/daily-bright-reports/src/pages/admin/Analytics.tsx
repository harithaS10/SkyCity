  ...
  return (
    <DashboardLayout>
      <div className="w-full">
        ...
        <div className="hidden lg:block space-y-8 animate-in fade-in duration-500 pt-2">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <Card className="border-none shadow-lg"><CardHeader><CardTitle>Productivity Breakdown</CardTitle></CardHeader><CardContent><div className="h-80 w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={employeeStats} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} barGap={12}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" opacity={0.2} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 12, fontWeight: 600, opacity: 0.8 }} dy={10} /><YAxis axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 12, opacity: 0.8 }} dx={-10} /><Tooltip cursor={{ fill: 'rgba(0,0,0,0.04)' }} contentStyle={{ borderRadius: '12px', border: 'none', backgroundColor: 'hsl(var(--card))', color: 'hsl(var(--card-foreground))' }} /><Legend wrapperStyle={{ paddingTop: '20px' }} iconType="circle" /><Bar dataKey="completed" name="Done" fill="#10b981" radius={[4, 4, 0, 0]} barSize={32} /><Bar dataKey="pending" name="Pending" fill="#f59e0b" radius={[4, 4, 0, 0]} barSize={32} /></BarChart></ResponsiveContainer></div></CardContent></Card>
            <Card className="border-none shadow-lg"><CardHeader><CardTitle>Work Distribution</CardTitle></CardHeader><CardContent><div className="h-80 w-full"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={workTypeDistribution} cx="50%" cy="50%" innerRadius={80} outerRadius={100} paddingAngle={5} dataKey="value" label={({ name, percent, x, y }) => percent < 0.05 ? null : <text x={x} y={y} fill="currentColor" fontSize={11} fontWeight={700} textAnchor="middle" dominantBaseline="central" opacity={0.9}>{`${name.slice(0, 12)} ${(percent * 100).toFixed(0)}%`}</text>}>{workTypeDistribution.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip contentStyle={{ borderRadius: '12px', backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', color: 'hsl(var(--card-foreground))' }} /></PieChart></ResponsiveContainer></div></CardContent></Card>
          </div>
        ...
        <div className="block lg:hidden min-h-screen bg-slate-50 dark:bg-slate-950 pb-[100px] -mx-4 -mt-4 animate-in fade-in duration-300">
          ...
          <div className="px-4 -mt-5 relative z-20 space-y-4">
            ...
            <Card className="border-none shadow-lg rounded-[1.5rem] overflow-hidden">
               ...
               <CardContent className="p-2"><div className="h-48 w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={employeeStats} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="currentColor" opacity={0.1} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 8, fontWeight: 700, opacity: 0.6 }} /><YAxis axisLine={false} tickLine={false} tick={{ fill: 'currentColor', fontSize: 8, opacity: 0.6 }} /><Tooltip contentStyle={{ borderRadius: '12px', fontSize: '10px', backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))' }} /><Bar dataKey="completed" fill="#10b981" radius={[3, 3, 0, 0]} barSize={10} /><Bar dataKey="pending" fill="#f59e0b" radius={[3, 3, 0, 0]} barSize={10} /></BarChart></ResponsiveContainer></div></CardContent>
            </Card>
